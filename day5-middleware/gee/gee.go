...
func (group *RouterGroup) Use(middlewares ...HandlerFunc) {
	group.middlewares = append(group.middlewares, middlewares...) // ...是一个语法糖，这里表示将middlewares一个个打散进行append
}


// 接收到一个具体请求时，要判断请求适用于哪些中间件（这里用URL prefix来判断）
...
func (engine *Engine) ServeHTTP(w http.ResponseWriter, req *http.Request){ // 解析请求的路径，查找路由映射表，查到就执行注册的处理方法；若查不到，返回404
	...
	engine.router.handle(c) // handle函数中，将从路由匹配得到的Handler添加到c.handlers列表中，执行c.Next
}
